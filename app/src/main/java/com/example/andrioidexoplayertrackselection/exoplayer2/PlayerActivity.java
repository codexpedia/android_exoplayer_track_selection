 ...
 */
package com.example.andrioidexoplayertrackselection.exoplayer2;
...
import com.example.andrioidexoplayertrackselection.track_selection_dialog.TrackSelectionDialog;
...
public class PlayerActivity extends AppCompatActivity
  ...
  private Button logTracksButton;
  private Button showTextCaptionsButton;
  private Button removeTextCaptionsButton;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    logTracksButton = findViewById(R.id.log_tracks);
    showTextCaptionsButton = findViewById(R.id.show_text_caption);
    removeTextCaptionsButton = findViewById(R.id.remove_text_caption);
    initDebugViews();
  ...
  private void initializePlayer() {
    if (player == null) {
      ...
      RenderersFactory renderersFactory = VideoDownloadManager.getInstance(getApplicationContext()).buildRenderersFactory(preferExtensionDecoders);
  ...
  private MediaSource buildMediaSource(Uri uri, @Nullable String overrideExtension) {
    Log.d(TAG, "buildMediaSource uri: " + uri.toString()  + " overrideExtension: " + overrideExtension);
  ...
  private DefaultDrmSessionManager<FrameworkMediaCrypto> buildDrmSessionManagerV18(UUID uuid, String licenseUrl, String[] keyRequestPropertiesArray, boolean multiSession) throws UnsupportedDrmException {
    HttpDataSource.Factory licenseDataSourceFactory = VideoDownloadManager.getInstance(getApplicationContext()).buildHttpDataSourceFactory();
  ...
  private void updateButtonVisibility() {
    if (player != null && TrackSelectionDialog.willHaveContent(trackSelector)) {
      ...
      logTracksButton.setEnabled(true);
      showTextCaptionsButton.setEnabled(true);
      removeTextCaptionsButton.setEnabled(true);
    } else {
      selectTracksButton.setEnabled(false);
      logTracksButton.setEnabled(false);
      showTextCaptionsButton.setEnabled(false);
      removeTextCaptionsButton.setEnabled(false);
    }
  }
